...
**Definition** — In `follow` mode, the auto-zoom camera never shows a view wider than `Camera.MAX_VIEW_TILES` (20) tiles, however far apart the framing targets spread. It is a floor on the derived `scale`, applied after the existing map-bounds/padding logic in `Camera.computeFraming`, so a map narrower than the cap is unaffected and still frames flush with its normal padding behaviour.

**Boundary** — Applies only to `follow` mode. Overview, level start, and game-over all reach `Camera.fullMapView` uncapped (the full map, whatever its size) — `Camera:computeTargetView` returns that view directly for `overview`/`gameover` without ever passing the cap into `computeFraming`.
...
**Definition** — A dynamic 2-player split-screen camera system that transitions continuously from a unified, dynamically zoomed camera to a dual-canvas Voronoi directional split. Uses a GLSL fragment shader to composite two full-window off-screen canvases (one per player) by signed distance to an explicit dividing **line** — a point and unit normal in screen pixels, always through screen centre, only rotating (see Split angle) — rather than by distance to either player's projected position; `CameraManager` computes that line once and the shader consumes it directly, so the two canvases and the line it draws between them can never disagree about where a player is. The split decision is a crossover trigger, not a distance or framing trigger: `CameraManager:updateSplit` compares the players' on-screen separation (world separation × the merged camera's eased zoom) against the on-screen separation of the two split regions' centroids — the pixels the split view would draw those same players at (see Split region). Once the players are drawn further apart than their own split positions, the screen splits; once they are drawn closer together than them, it merges. Because both renderings place both players identically at that point, neither the split nor the merge moves anything on screen: the screen appears to part as the players walk away from each other and close up as they walk back together. Two named constants (`SPLIT_TRIGGER_SEPARATION_MARGIN`, `SPLIT_RELEASE_SEPARATION_MARGIN`) put the merge-off threshold slightly inside the split-on one, a deliberately narrow dead band that exists only to stop the state thrashing frame-to-frame on the boundary. The follow zoom cap governs how far the merged camera may zoom out, and so indirectly how far apart the players get before reaching the crossover, but is not itself part of the trigger.
...
**Definition** — A continuous value [0..1] driving the Voronoi split-screen transition. 0 = fully merged (single camera view), 1 = fully split (two independent per-player cameras composited by the Voronoi shader). Its target (0 or 1) is set by the crossover split trigger described under Voronoi split-screen above — not by a raw distance ratio — and the value itself is eased toward that target frame-to-frame via exponential decay (`1 - e^(-decay * dt)`) to prevent abrupt visual pops.
...
**Definition** — The dividing line always passes through screen centre and only rotates, so the split is two equal halves at every separation, including when the follow camera is clamped at a map edge. The angle itself is computed as `atan2(py2 - py1, px2 - py1)` from the two players' world positions. Smoothed frame-to-frame via exponential lerp with a per-frame maximum rotation clamp (~150 deg/s) and a minimum angle threshold (~5 deg) to prevent jitter when players are aligned — an aligned pair holds the previous angle rather than snapping. Uses shortest-path atan2 interpolation (adjusts by +/- pi to avoid spinning the long way around), so a full 360° rotation of the players produces a continuously rotating line with no sign flip.

`CameraManager:getSplitLine()` is the single owner of this geometry: it turns the angle into a point (screen centre) and a unit normal, in screen pixels, and that's what both `InGameState` and the Voronoi shader consume — the shader takes `line_point`/`line_normal` uniforms directly rather than deriving the line from player screen positions itself. The line's point can also slide off centre along the normal by a signed offset scalar; that offset is a documented seam (`CameraManager:_splitLineOffset`) shipped at zero today, for a later slice to drive.

**Boundary** — A camera-state value used only by the Voronoi shader compositing pass. It determines the orientation (and, via the offset seam, position) of the dividing line on screen but does not affect gameplay, physics, or entity behavior. The angle is computed in `CameraManager:updateSplit`; the line geometry itself is computed in `CameraManager:getSplitLine`.

## Split region

**Definition** — The convex area each player's pane camera anchors its player within: the half-plane on that player's side of the split line, intersected with the screen rect. Its centroid — computed with a general convex-polygon (area-weighted / shoelace) formula, not a per-shape special case, since clipping a rect by a half-plane can yield a triangle, quadrilateral, or pentagon depending on the line's angle — is the point each pane's player is anchored to on screen. Because a half-plane ∩ rect is always convex, and a convex region's area-weighted centroid always lies strictly inside it, a player can never end up on the far side of the dividing line, at any separation or angle — that guarantee comes from the geometry, not from tuning. `CameraManager.computeRegionCentroid(screenW, screenH, line, side)` is the pure, static helper (`side = -1` for pane 1 / CanvasA, matching the shader's `sd<=0`; `side = 1` for pane 2 / CanvasB); `CameraManager:getPaneAnchor(index)` is the eased instance-level anchor `getPaneDrawParams` actually uses. The anchor itself is eased (not the camera centre it combines with) as the line rotates, so the world doesn't visibly slosh on a fast turn. Degenerate inputs (a zero-length line normal, or a zero-area screen rect) return the screen centre rather than a NaN.

**Boundary** — A camera-state value, purely geometric. `CameraManager` is the single owner of this geometry (alongside the split line itself); `InGameState` and the Voronoi shader only consume the resulting draw params, never re-derive the region or its centroid.
