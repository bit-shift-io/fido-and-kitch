<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.11" tiledversion="1.12.2" name="props" tilewidth="714" tileheight="714" tilecount="13" columns="0">
 <grid orientation="orthogonal" width="1" height="1"/>
 <tile id="0">
  <image source="../img/default.png" width="32" height="32"/>
 </tile>
 <tile id="1">
  <image source="../img/pushable_crate_wood.png" width="128" height="128"/>
 </tile>
 <tile id="2" x="96" y="0" width="32" height="32">
  <image source="../img/ladder.png" width="128" height="32"/>
 </tile>
 <tile id="3">
  <image source="../img/entity_switch.png" width="512" height="512"/>
 </tile>
 <tile id="4">
  <image source="../img/entity_key.png" width="256" height="256"/>
 </tile>
 <tile id="5" x="0" y="0" width="512" height="512">
  <image source="../img/entity_exit_door.png" width="1024" height="512"/>
 </tile>
 <tile id="6" x="0" y="0" width="20" height="20">
  <image source="../img/coins.png" width="160" height="20"/>
 </tile>
 <tile id="8">
  <image source="../img/entity_teleporter.png" width="512" height="512"/>
 </tile>
 <tile id="9" x="0" y="0" width="380" height="370">
  <image source="../img/cage/cage.png" width="760" height="370"/>
 </tile>
 <tile id="10" x="0" y="0" width="256" height="128">
  <image source="../img/entity_jump_pad.png" width="768" height="128"/>
 </tile>
 <tile id="11">
  <image source="../img/default.png" width="32" height="32"/>
 </tile>
 <tile id="12">
  <image source="../img/entity_wood_sign_post.png" width="128" height="128"/>
 </tile>
 <tile id="13">
  <image source="../img/entity_mover_platform.png" width="714" height="714"/>
 </tile>
</tileset>
