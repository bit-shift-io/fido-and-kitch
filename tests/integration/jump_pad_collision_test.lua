...
test('a path blocked by a solid tile ends in FallState, never entering the wall', function()
	...
	assertEqual('WalkIdleState', player.fsm.currentState.name, 'expected the player to land normally after the blocked exit')
end)
...
test('regression: an unblocked jump-pad path completes exactly as before', function()
	...
	assertTrue(stateName == 'WalkIdleState' or stateName == 'FallState',
