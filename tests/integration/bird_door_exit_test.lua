...
test('a following bird leaves FollowState and flies to the door once it opens', function()
	...
	assertEqual('FlyToDoorState', bird.stateMachine.currentState.name,
...
test('a following rabbit is unaffected by the door opening beyond the existing despawn radius (regression)', function()
	...
	FrameStepper.step(game, 30)
	assertEqual('FollowState', rabbit.stateMachine.currentState.name,
	...
	assertTrue(rabbit.stateMachine.currentState.name ~= 'FlyToDoorState',
		'rabbit should never enter FlyToDoorState')
end)
