...
test("a push_box blocks the player -- walking into it cannot pass through", function()
	...
	assertTrue(
		Queries.playerPositionV(player).x > startX + 50,
...
test("the player can stand on top of a push_box as if it were solid ground", function()
	...
	assertNear(
		BOX_TOP - PLAYER_HALF_HEIGHT,
		Queries.playerPositionV(player).y,
		1,
		"expected the player to rest on the box's top, not sink through it"
	)
	assertTrue(
		player:queryOnGround(),
...
test("the box stops the instant the player releases the direction", function()
	...
	assertNear(restingX, box.collider:getX(), 0.001, "expected the box to stop dead on release, with no coasting")
end)
...
test("an airborne player cannot push the box", function()
	...
	assertNear(boxX, box.collider:getX(), 0.001, "expected a mid-air player to be blocked by the box, not to push it")
end)
...
test("a box with a player standing on it cannot be pushed", function()
	...
	assertNear(boxX, box.collider:getX(), 0.001, "expected a box with a player aboard to refuse to move by default")
end)
...
test("a box with allowPushWhenStoodOn set can be pushed with a player aboard", function()
	...
	assertTrue(box.collider:getX() > boxX + 20, "expected the opted-in box to slide even with a player standing on it")
end)
...
test("a box with another pushable resting on it cannot be pushed", function()
	...
	assertNear(boxX, box.collider:getX(), 0.001, "expected a box with a prop stacked on it to refuse to move")
end)
...
test("pushing a box into a wall stops it at the wall", function()
	...
	assertNear(WALL_FACE - 16, box.collider:getX(), 1.5, "expected the box to come to rest flush against the wall")
end)
...
test("a box pushed over a one-tile hole snaps into it and fills it flush", function()
	...
	assertNear(HOLE_CENTRE_X, box.collider:getX(), 0.5, "expected the box to align to the hole's tile centre")
	assertNear(
		HOLE_FLOOR_TOP - 16,
		box.collider:getY(),
		1,
...
test("a box resting over solid ground keeps its arbitrary x -- no grid alignment", function()
	...
	assertTrue(
		math.abs(offsetWithinTile - map.tilewidth * 0.5) > 1,
...
test("a box in mid-air cannot be pushed, and is pushable again once it lands", function()
	...
	for _ = 1, 6 do
		FrameStepper.step(game, 1)
		assertNear(boxX, box.collider:getX(), 0.001, "expected a falling box to ignore the push and drop straight down")
	end

	FrameStepper.step(game, 45) -- land, then keep pushing
	controller:release("right")

	assertTrue(box.collider:getX() > boxX + 10, "expected the box to become pushable again once it landed")
end)
...
test("a box filling a hole is solid ground the player can walk across", function()
	...
	assertTrue(
		Queries.playerPositionV(player).x > 320,
...
test("a box pushed into a bottomless water gap sinks to the map's bottom border", function()
	...
	assertNear(MAP_BOTTOM - 16, box.collider:getY(), 1, "expected the box to come to rest on the map's bottom boundary")
	assertTrue(
		box.collider:getX() > 160 and box.collider:getX() < 192,
