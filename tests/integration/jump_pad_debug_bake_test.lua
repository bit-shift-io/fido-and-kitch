...
test("conf.debug true: a target-linked, path-less jump pad gets a working path at entity-init time", function()
	...
	withDebug(true, function()
		...
		assertTrue(
			pad.pathObject.polyline ~= nil and #pad.pathObject.polyline > 0,
			"baked path should carry a non-empty polyline"
		)
	end)
	...
	assertTrue(
		findProperty(bakedPad, "path") ~= nil,
		"the bake should have written a path property back to the file on disk"
	)
