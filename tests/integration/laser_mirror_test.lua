...
test("a laser bent around a corner by one mirror destroys a boulder a straight beam could never reach", function()
	...
	assertTrue(
		Queries.findEntityByName(map, "boulder_bounce_target") ~= nil,
...
test(
	"a mirror on the other diagonal (flipMirror=false) redirects the same rightward beam upward instead of downward",
	function()
		local game = GameHarness.startGame(MAP)
		...
		assertTrue(
			Queries.findEntityByName(map, "boulder_alt_target") ~= nil,
...
test("a lever switch linked to a mirror flips it mid-beam, changing which target the same laser hits", function()
	...
	assertTrue(
		laser.beamHitEntity == targetDown,
