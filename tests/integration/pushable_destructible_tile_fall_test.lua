...
test('a push_box resting on a destructible tile falls and settles on the floor below once the tile is destroyed', function()
	...
	assertNear(BOX_ON_TILE_Y, box.collider:getY(), 1,
	...
	assertNear(BOX_ON_FLOOR_Y, box.collider:getY(), 1,
		'expected the box to have fallen through the gap and landed on the floor below')
	assertEqual('static', box.collider.bodyType,
		'expected the box to be back to a static body once it has landed')
end)
