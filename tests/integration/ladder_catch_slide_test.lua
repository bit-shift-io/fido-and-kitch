...
test("a fall-catch while holding left hangs instead of sliding out of the ladder", function()
	...
	assertTrue(math.abs(x - CENTRE_X) < 16, "the player must remain inside the ladder volume (x=" .. tostring(x) .. ")")
...
test("releasing the slide key holds position instead of re-centring", function()
	...
	assertTrue(
		math.abs(Queries.playerPositionV(player).x - releasedX) <= 2,
		"expected position to hold after release (released at x="
			.. tostring(releasedX)
			.. ", now x="
			.. tostring(Queries.playerPositionV(player).x)
			.. ")"
	)
end)
...
test("a side press while climbing (up held) is ignored, keeping the player on the ladder", function()
	...
	assertTrue(
		math.abs(x - xAtPress) <= 1,
		"side press must be ignored while up is held (x at press="
			.. tostring(xAtPress)
			.. ", after climbing x="
			.. tostring(x)
			.. ")"
