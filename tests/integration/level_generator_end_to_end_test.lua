...
test('every mechanic together (large, difficulty 5, coop required) still loads and is completable via cages alone', function()
	...
	local ok, err = pcall(function()
		...
		assertTrue(exitDoors[1].usable.enabled, 'expected the exit to open once every cage is used, with every other mechanic present')
	end)
