...
test('a player standing on the deck is carried by the exact platform delta', function()
	...
	for _ = 1, 40 do
		...
		if math.abs(platDelta) > 0.001 then
			riding = true
			assertNear(platDelta, plyDelta, 0.002,
				'expected the rider to move by the exact platform delta, got platDelta=' .. platDelta .. ', plyDelta=' .. plyDelta)
		end
	end
	assertTrue(riding, 'expected the platform to be moving at some point during the carry check')
	assertNear(PLAYER_RESTING_Y, player.collider:getPositionV().y, 2,
...
test('the linked switch stops the platform in place and restarting it resumes', function()
	...
	assertNear(frozenX, platform.collider:getPositionV().x, 0.001,
		'expected the platform to stay in place while the switch is off')
	assertNear(frozenY, platform.collider:getPositionV().y, 0.001,
		'expected the platform to stay in place while the switch is off')
	...
	assertTrue(platform.collider:getPositionV().x ~= resumedX,
