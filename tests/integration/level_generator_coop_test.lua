...
local GENERATED_PATH = "res/map/generated/_test_coop.tmj"
...
test("the vault teleport starts disabled and only the pressure plate can enable it, momentarily", function()
	...
	local ok, err = pcall(function()
		...
		plate:update(1 / 60)
		assertFalse(
			vaultEntrance.usable.enabled,
			"expected the plate to disable the vault teleport the instant the weight leaves"
		)
	end)
