...
test(
	"a Spider that dies while it has a player wrapped releases them immediately, well short of the web expiry",
	function()
...
test(
	"a Spider that dies with no one wrapped behaves identically to a Robot -- dies and stays dead, no error",
	function()
		local game = GameHarness.startGame(MAP)
