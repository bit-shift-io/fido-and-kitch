...
test("levelTimer increases by dt each update frame", function()
	...
	assertNear(
		...
		0.0001,
		string.format(
			"after %d frames, levelTimer should be ~%.4f, got %.4f",
			framesToRun,
			expectedTime,
			state.levelTimer
		)
...
test("levelTimer continues accumulating during gameOverTimer (death zoom)", function()
	...
	assertNear(
		...
		actualAccumulation,
		0.0001,
		string.format("levelTimer should accumulate by ~%.4f, got %.4f", expectedAccumulation, actualAccumulation)
	)
end)
...
test("levelTimer accumulates correctly over extended gameplay", function()
	...
	assertNear(
		...
		state.levelTimer,
		0.01,
		string.format("after 5 seconds, levelTimer should be ~%.2f, got %.4f", expectedTime, state.levelTimer)
	)
end)
