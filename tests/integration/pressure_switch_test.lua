...
test("a player brushing the plate's edge does not activate it", function()
	...
	assertFalse(Queries.pressureSwitchIsActive(plate), "expected a weight only clipping the plate edge to leave it off")
end)
...
test("activating the plate drives its target through the switch mechanism", function()
	...
	assertEqual(heightBefore + 2, ladder:tileHeight(), "expected the plate to drive its target, growing the ladder")
end)
...
test("a momentary plate releases when the weight leaves", function()
	...
	assertFalse(Queries.pressureSwitchIsActive(plate), "expected a momentary plate to follow the weight and release")
end)
...
test("a latching plate stays on after the weight leaves", function()
	...
	assertTrue(Queries.pressureSwitchIsActive(plate), "expected a latching plate to hold once tripped")
end)
...
test("two weights count as one activation, and it releases only when the last leaves", function()
	...
	stand(p1, 64)
	FrameStepper.step(game, 5)
	assertTrue(Queries.pressureSwitchIsActive(plate), "expected the plate to stay on while one weight remains")

	stand(p2, 96)
	FrameStepper.step(game, 5)
	assertFalse(Queries.pressureSwitchIsActive(plate), "expected the plate to release once the last weight left")
end)
