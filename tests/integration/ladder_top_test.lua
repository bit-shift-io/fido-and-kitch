...
local function mountToTop(game, controller, player)
	runUntil(game, function()
		return player.fsm.currentState.name == 'WalkIdleState'
			and math.abs(feetY(player) - TOP_EDGE_Y) <= 3
	end, 180)

	controller:press('down')
	runUntil(game, function()
		return player.fsm.currentState.name == 'LadderState'
			and feetY(player) > TOP_EDGE_Y + 20
...
test('a player climbing out of the volume stands on the bare ladder top with no terrain beneath', function()
	...
	assertTrue(math.abs(feetY(player) - TOP_EDGE_Y) <= 3,
...
test('a player falling onto the bare ladder top from above lands standing on it', function()	local game = GameHarness.startGame(MAP)
	local player = player1(game)
...
test('pressing down while standing on the bare top descends into the ladder volume', function()
	...
	runUntil(game, function()
		return player.fsm.currentState.name == 'LadderState'
			and feetY(player) > TOP_EDGE_Y + 24
	end, FrameStepper.secondsToFrames(3))
	controller:release('down')
	...
	assertEqual('LadderState', player.fsm.currentState.name, 'expected to keep hanging inside the volume after releasing down')
	assertTrue(math.abs(feetY(player) - hangY) <= 2,
...
test('sliding off the top dismounts directly without a fall or landing sound', function()
	...
	for _, name in ipairs(spy.played) do
		assertFalse(name == 'land', 'expected no landing sound on a direct dismount (played: '
			.. table.concat(spy.played, ',') .. ')')
	end
end)
