...
test('a boulder keeps rolling at walk speed after the player stops touching it', function()
	...
	assertNear(playerXAtRelease, Queries.playerPositionV(player).x, 1,
		'fixture check: the player should have stopped walking after releasing')
	assertNear(player.speed * 0.5, travelled, 3,
...
test('a rolling boulder stops harmlessly at a wall', function()
	...
	assertNear(WALL_FACE + 16, boulder.collider:getX(), 1.5,
		'expected the boulder to come to rest flush against the wall')
	assertNear(BOULDER_RESTING_Y, boulder.collider:getY(), 1,
...
test('a rolling boulder stops when it reaches a player, without shoving them', function()
	...
	assertNear(blockerX - PLAYER_HALF_WIDTH - 16, boulder.collider:getX(), 2,
		'expected the boulder to stop at the player it reached')
	assertNear(blockerX, blocker.collider:getX(), 1.5,
...
test('a rolling boulder falls and snaps into a hole like a box', function()
	...
	assertNear(HOLE_CENTRE_X, boulder.collider:getX(), 0.5,
		'expected the boulder to align to the hole\'s tile centre')
	assertNear(HOLE_FLOOR_TOP - 16, boulder.collider:getY(), 1,
		'expected the boulder to come to rest filling the hole')
end)
...
test('a boulder that has stopped can be pushed again, back the way it came', function()
	...
	assertTrue(boulder.collider:getX() < stoppedX - 20,
