...
test("pressing down on a seam perch descends the down-leading ladder", function()
	...
	assertEqual("LadderState", player.fsm.currentState.name, "expected to be descending inside the down-leading ladder")
	local pos = Queries.playerPositionV(player)
	assertTrue(pos.x >= 33 and pos.x <= 63, "expected to end up centred in column B (x=" .. tostring(pos.x) .. ")")
end)
...
test("releasing keys while hanging beside a platform crossing stays mounted", function()
	...
	assertEqual("LadderState", player.fsm.currentState.name, "releasing keys mid-column must not dismount the climber")
	local bottom = player.collider:getBounds().bottom
	assertTrue(
		bottom > 195,
