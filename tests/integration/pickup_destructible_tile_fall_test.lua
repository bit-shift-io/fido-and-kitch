...
test(
	"a coin and a key resting on a destructible tile each fall and settle on the floor below once the tile is destroyed",
	function()
		...
		assertEqual("static", key.collider.bodyType, "expected the key to be back to a static body once it has landed")
	end
)
...
test("a coin stays collectible mid-fall", function()
	...
	FrameStepper.step(game, 5)
	assertTrue(
		math.abs(coin.collider:getY() - PICKUP_ON_FLOOR_Y) > 5,
	...
	assertTrue(
		player.inventory:hasItems("coin", 1),
