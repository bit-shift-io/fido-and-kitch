-- Unit tests for swoop curve generation math
local swoop_curve = require("src.utils.swoop_curve")

test("computeCurvePoint returns start position at t=0", function()
	local start = { x = 100, y = 200 }
	local dest = { x = 500, y = 200 }
	local curve = swoop_curve.generateCurve(start, dest)
	local p = swoop_curve.computeCurvePoint(curve, 0)
...
test("computeCurvePoint returns dest position at t=1", function()
	...
	local dest = { x = 500, y = 200 }
	local curve = swoop_curve.generateCurve(start, dest)
	local p = swoop_curve.computeCurvePoint(curve, 1)
...
test("computeCurvePoint at t=0.5 has y offset for horizontal travel", function()
	...
	local dest = { x = 500, y = 200 }
	local curve = swoop_curve.generateCurve(start, dest)
	local p = swoop_curve.computeCurvePoint(curve, 0.5)
...
test("generateCurve produces deterministic results for same inputs", function()
	local start = { x = 100, y = 200 }
	local dest = { x = 500, y = 200 }
	local curve1 = swoop_curve.generateCurve(start, dest)
	local curve2 = swoop_curve.generateCurve(start, dest)
	for t = 0, 1, 0.1 do
		local p1 = swoop_curve.computeCurvePoint(curve1, t)
		local p2 = swoop_curve.computeCurvePoint(curve2, t)
...
test("curve points stay close to straight line (bounded wobble)", function()
	...
	for t = 0, 1, 0.05 do
		local p = swoop_curve.computeCurvePoint(curve, t)
...
test("calculateTravelDuration scales with distance", function()
	local d1 = swoop_curve.calculateTravelDuration(100)
	local d2 = swoop_curve.calculateTravelDuration(500)
	local d3 = swoop_curve.calculateTravelDuration(1000)
...
test("calculateTravelDuration clamps to min/max", function()
	local minDur = swoop_curve.calculateTravelDuration(0)
	local maxDur = swoop_curve.calculateTravelDuration(10000)
...
test("calculateTravelDuration formula: base + 0.0025s per pixel", function()
	local base = 0.5
	local perPx = 0.0025
	local d = swoop_curve.calculateTravelDuration(100)
	local expected = base + perPx * 100
	assertNear(expected, d, 0.01, "formula matches at 100px")
end)

test("TeleportTrail delegates to SwoopCurve.generateCurve", function()
	local teleport_trail = require("src.fx.teleport_trail")
	local start = { x = 100, y = 200 }
	local dest = { x = 500, y = 200 }

	-- Call through TeleportTrail
	local curve = teleport_trail.generateCurve(start, dest)

	-- Verify it returns a curve object with expected structure
	assertTrue(curve.startX == start.x, "curve has startX")
	assertTrue(curve.startY == start.y, "curve has startY")
	assertTrue(curve.destX == dest.x, "curve has destX")
	assertTrue(curve.destY == dest.y, "curve has destY")
	assertTrue(curve.dist > 0, "curve has dist")
end)

test("TeleportTrail delegates to SwoopCurve.computeCurvePoint", function()
	local teleport_trail = require("src.fx.teleport_trail")
	local start = { x = 100, y = 200 }
	local dest = { x = 500, y = 200 }
	...
	-- Verify it returns a point
	assertTrue(p.x ~= nil, "point has x")
	assertTrue(p.y ~= nil, "point has y")
	assertTrue(p.x > 0 and p.y > 0, "point has valid coordinates")
end)

test("TeleportTrail delegates to SwoopCurve.calculateTravelDuration", function()
	local teleport_trail = require("src.fx.teleport_trail")

	local d = teleport_trail.calculateTravelDuration(100)

	-- Verify it returns a duration
	assertTrue(d >= 0.5 and d <= 3.0, "duration is in valid range")
end)
