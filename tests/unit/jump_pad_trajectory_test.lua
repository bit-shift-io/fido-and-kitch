...
test("a close, level jump still clears 2 tiles above both endpoints", function()
	...
	assertTrue(
		peakY <= highestEndpointY - 64,
		string.format(
			"expected peak y <= %d (2 tiles above the higher endpoint), got %.1f",
			highestEndpointY - 64,
			peakY
		)
	)
...
test(
	"minimum clearance is measured against the HIGHER of the two endpoints, even when they differ in height",
	function()
