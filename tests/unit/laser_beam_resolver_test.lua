...
test("beam stops (opaque) at a laser_switch hit from its accepted direction, and records it as activated", function()
	...
	assertTrue(
		result.hitEntity == switchEntity,
		"expected the switch to stop the beam, same as any other solid obstacle"
	)
...
test(
	"beam still stops (opaque) at a laser_switch hit from the WRONG direction, but does not record it as activated",
	function()
		local switchEntity = {
			type = "laser_switch",
			acceptsDirection = function(self, incomingDirection)
		...
		assertTrue(
			result.hitEntity == switchEntity,
			"expected the switch to still stop the beam even though it did not activate"
		)
...
test("a beam bounces through a two-mirror chain to reach its target", function()
	...
	local mirrorB = {
		type = "mirror",
		redirect = function(self, incomingDirection)
			if incomingDirection == "right" then
				return "down"
			end
			return nil
		end,
...
test("a looping mirror pair terminates at the bounce cap instead of recursing forever", function()
	...
	assertTrue(
		result.hitEntity == mirrorA or result.hitEntity == mirrorB,
	...
	assertEqual(maxBounces + 1, #result.segments, "expected exactly maxBounces + 1 segments before the beam gives up")
end)
