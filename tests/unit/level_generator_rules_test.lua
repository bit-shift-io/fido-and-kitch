...
test('every applicable rule returns objects, idsUsed, and a walkthroughStep', function()
	...
	for _, rule in ipairs(rules) do
		if rule.canApply(layout) then
			...
			assertTrue(type(result.walkthroughStep) == 'string' and #result.walkthroughStep > 0, rule.id .. ' missing a walkthrough step')

			-- object ids must all fall within [startId, startId + idsUsed) with no collisions
			local seen = {}
			for _, object in ipairs(result.objects) do
				assertTrue(object.id >= 500 and object.id < 500 + result.idsUsed, rule.id .. ' emitted an id outside its declared range')
