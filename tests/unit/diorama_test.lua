		...
		frame = {
			...
			ornaments = {
				...
				top = {
					...
					{
						img = "res/img/diorama/diorama_orn_top_b.png",
						pos = 2 / 3,
						scale = { x = -1, y = 0.75 },
						offset = 10,
					},
				...
				right = {
					{
						img = "res/img/diorama/diorama_orn_right_a.png",
						pos = 0.25,
						scale = { x = 1, y = 1 },
						offset = 6,
					},
...
test("config defaults are present and stable", function()
	...
	for _, key in ipairs({ "topLeft", "topRight", "bottomLeft", "bottomRight" }) do
		...
		assertTrue(
			type(scale) == "table" and type(scale.x) == "number" and type(scale.y) == "number",
			"frame.ornaments.corners." .. key .. ".scale should be an {x, y} pair"
		)
		if orn.offset then
			assertTrue(
				type(orn.offset) == "number",
				"frame.ornaments.corners." .. key .. ".offset should be a number of world px"
			)
		end
	end
	for _, side in ipairs({ "top", "bottom", "left", "right" }) do
		...
		assertTrue(
			type(orn) == "table" and #orn > 0,
			"frame.ornaments." .. side .. " should be a non-empty list of ornaments"
		)
		for _, item in ipairs(orn) do
			assertTrue(type(item.img) == "string", "ornament img should be a path string")
			assertTrue(
				type(item.pos) == "number" and item.pos >= 0 and item.pos <= 1,
				"ornament pos should be a 0..1 fraction along the edge"
			)
			if item.scale then
				assertTrue(
					type(item.scale.x) == "number" and type(item.scale.y) == "number",
					"ornament scale should be an {x, y} pair"
				)
			end
			if item.offset then
...
test("void rects cover all four sides when the world is fully inset", function()
	...
	for _, r in ipairs(rects) do
		assertTrue(
			r.x >= 0 and r.y >= 0 and r.x + r.w <= 1000 and r.y + r.h <= 700,
			"strip must stay inside the screen"
		)
