...
A standalone, offline level generator (not part of the shipped game) that emits complete, playable-but-bland levels as Tiled `.tmx` files (the game loads `.tmx` directly, no export step needed), along with a human-readable solution walkthrough for each level. Designers run a seeded CLI, playtest candidates immediately, open the keepers in Tiled to hand-tweak, and save straight back to a playable file. Every generated level is solvable **by construction**: the generator builds the solution path first and decorates around it, so no design time is wasted discovering broken levels.
...
4. As a level designer, I want the generated `.tmx` to be immediately playable, so that I can playtest a candidate immediately (`love . map=...`) without opening Tiled first.
...
10. As a player of a generated level, I want every key, cage, switch, and the exit to be reachable with the game's actual movement (walking and ladders — the game has no jump), so that the level is completable without glitches.
...
14. ~~As a level designer, I want released birds to sometimes perform useful actions on their path (e.g. flick a remote switch) before exiting...~~ **Deferred** — the bird NPC has no path-following or `object:exec` capability in the game yet (DECISIONS.md Q13); this story is blocked on that shipping, the same way pushables/enemies were blocked until they landed.
...
- **Reads gameplay constants from `src/`** (tile size, player walk/climb speed, pushable rules) rather than duplicating numbers that would drift. Where a constant is not cleanly requirable, extract a testability seam in `src/` rather than copying the value.
- **Solution-first construction**: the generator first builds an abstract solution plan — objectives (key → cage → … → exit) placed into zones the movement model already guarantees are connected — then realises terrain, then decorates. Solvability is never checked after the fact; it is implied by construction.
- **Movement model**: a conservative reachability model derived from the game's real traversal mechanics. **The player has no jump** (verified against `src/player/` — walking, ladder climbing, and authored jump-pad paths/drawbridges/pushables are the only ways to move, no jump input/impulse/state exists). Reachability is therefore walk-adjacency within a zone plus ladder edges between zones (v1); jump pads, drawbridges, and pushables extend reachability in their respective later issues. The generator only asserts traversals the model guarantees — never a bare gap crossing.
- **Modular puzzle-rule library**: each puzzle pattern (e.g. "lever switch opens remote door", "momentary pressure switch + separated door", "push box into hole to bridge a gap"; "bird flicks switch on exit path" deferred, see story 14) is a self-contained rule module with a uniform interface: what it requires, what it unlocks, how it expands into entities/terrain, and the walkthrough steps it contributes. New props → new rule files, no core changes.
...
- **Objective spine (corrected 2026-08-08, DECISIONS.md Q13)**: colored keys unlock matching cages; each cage releases a bird (cosmetic — it follows the releasing player, no path/exec); the exit opens automatically once every cage has been used, via the game's real `cage_unlocked`/`all_cages_unlocked` mechanism (`src/states/ingame_state.lua` + `src/entities/exit_door.lua`) — not the `actor_count` `Variable`, which is dead code.
- **Output**: `.tmx` referencing the project's existing tilesets/templates by relative path, plus a solution walkthrough text file, all written to a generated-maps directory under `res/map/`. The game loads `.tmx` directly (`src/map/init.lua`), so no Tiled CLI, no export step, and no degrade-without-Tiled path are needed.
...
- **Movement-model tests** assert agreement with real constants and mechanics from `src/` (e.g. the model never claims a gap crossing the player's actual walk/ladder mechanics can't make — there is no jump); prior art: existing gameplay regression tests in `tests/`.
...
```
tools/
  generate.sh              # launcher (arg passthrough, finds LuaJIT)
  level_generator/
    main.lua               # CLI parsing, batch loop, output orchestration
    rng.lua                # seeded PRNG
    movement_model.lua     # reachability maths fed by src/ constants (walk + ladder only, no jump)
...
res/map/generated/         # output: NN-seed.tmx, NN-seed-solution.md
```

## Acceptance Criteria

- [ ] `tools/generate.sh --seed 42` produces a `.tmx` and a solution walkthrough; `love . map=generated/<name>.tmx` loads and plays it.
...
- [ ] Every key, cage, switch, and the exit is reachable under the game's real movement (walking and ladders — no jump) according to the movement model.
...
- [ ] Headless tests for planner, movement model, TMX writer, and determinism pass via `./test-unit.sh`.

## References

- `.scratch/procedural-level-generation/DECISIONS.md` — authoritative rationale (solution-first construction, pushable motion/snap model); the ADRs referenced in earlier drafts of this doc were never actually written and `docs/` doesn't exist in this repo
