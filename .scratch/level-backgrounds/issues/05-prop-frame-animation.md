...
```lua
{
  ...
  template    = "firefly",
  count       = 20,
  region      = {x = 0, y = 0, w = "map_width", h = "map_height"},
  depth       = 0.3,
  windScale   = 0.0,
...
- Loader passes `animation` to prop entity; prop adds Sprite + Timeline when present.
- Motion (sway/shake) still applies to the drawn sprite (transform in draw).
- Demonstrate on one sandbox prop (placeholder frames OK — reuse coin frames or simple color shifts).
...
## Blocked by
03 and 04 (all background elements in place before adding animation demo). 04 recommended first so final visual pass covers everything.