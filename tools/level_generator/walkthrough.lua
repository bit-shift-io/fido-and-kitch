...
function Walkthrough.build(plan, flourishSteps)
	...
	for _, objective in ipairs(plan) do
		table.insert(lines, string.format("P1: take the %s key (zone %d).", objective.color, objective.keyZoneIndex))
		table.insert(
			lines,
			string.format("P1: use the %s cage (zone %d) to unlock it.", objective.color, objective.cageZoneIndex)
		)
	end
	table.insert(
		lines,
		"Once every cage has been used, the exit opens automatically -- walk both players through it to finish."
	)
