...
function Decoder:parseString()
	...
	while true do
		...
		if char == '' then self:error('unterminated string') end
		self.pos = self.pos + 1
		if char == '"' then
			break
		elseif char == '\\' then
...
function Decoder:parseNumber()
	...
	if not value then self:error('invalid number') end
...
function Decoder:parseArray()
	...
	while true do
		...
		if char ~= ',' then self:error("expected ',' or ']'") end
	end
	return result
end
...
function Decoder:parseValue()
	self:skipWhitespace()
	local char = self.text:sub(self.pos, self.pos)
	if char == '{' then return self:parseObject() end
	if char == '[' then return self:parseArray() end
	if char == '"' then return self:parseString() end
	if char == '-' or char:match('%d') then return self:parseNumber() end
