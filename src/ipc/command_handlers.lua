...
local function playerIndex(args, i)
	...
	if not idx or (idx ~= 1 and idx ~= 2) then
		return nil, "Player index must be 1 or 2"
	end
	return idx
end
...
local function resolveValidatedAction(raw)
	...
	if not validateAction(action) then
		return nil, "Action must be: left, right, up, down, use, start, back, a, or b"
	end
	return action
end
...
function CommandHandler:registerInfoCommands()
	self:register("GET_STATE", function(args)
		if not requireArgc(args, 0, "GET_STATE (no arguments)") then
			return nil, "Usage: GET_STATE (no arguments)"
		end
		return self.gameAPI.getState()
	end)

	self:register("GET_ENTITIES", function(args)
		if not requireArgc(args, 0, "GET_ENTITIES (no arguments)") then
			return nil, "Usage: GET_ENTITIES (no arguments)"
		end
		return self.gameAPI.getEntities()
	end)

	self:register("GET_TILE_GRID", function(args)
		if not requireArgc(args, 0, "GET_TILE_GRID (no arguments)") then
			return nil, "Usage: GET_TILE_GRID (no arguments)"
		end
		return self.gameAPI.getTileGrid()
	end)

	self:register("TOGGLE_DEBUG_DRAW", function(args)
		if not requireArgc(args, 0, "TOGGLE_DEBUG_DRAW (no arguments)") then
			return nil, "Usage: TOGGLE_DEBUG_DRAW (no arguments)"
		end
		return self.gameAPI.toggleDebugDraw()
	end)

	self:register("RESTART_LEVEL", function(args)
		if not requireArgc(args, 0, "RESTART_LEVEL (no arguments)") then
			return nil, "Usage: RESTART_LEVEL (no arguments)"
		end
		return self.gameAPI.restartLevel()
	end)

	self:register("MENU", function(args)
		if not requireArgc(args, 0, "MENU (no arguments)") then
			return nil, "Usage: MENU (no arguments)"
		end
		return self.gameAPI.goToMenu()
	end)

	self:register("TOGGLE_CAMERA", function(args)
		if not requireArgc(args, 0, "TOGGLE_CAMERA (no arguments)") then
			return nil, "Usage: TOGGLE_CAMERA (no arguments)"
		end
	...
	self:register("GET_LOG", function(args)
		if not _ipc_log then
			return nil, "Log buffer not available"
		end
...
function CommandHandler:registerPlayerCommands()
	self:register("MOVE_PLAYER", function(args)
		if not requireArgc(args, 3, "MOVE_PLAYER <1|2> <dx> <dy>") then
			return nil, "Usage: MOVE_PLAYER <1|2> <dx> <dy>"
		end
		local idx = assert(playerIndex(args, 1))
		local dx, dy = numberArg(args, 2, "dx"), numberArg(args, 3, "dy")
		if not dx or not dy then
			return nil, "All arguments must be numbers"
		end
		return self.gameAPI.movePlayer(idx, dx, dy)
	end)

	self:register("GET_PLAYER_POS", function(args)
		if not requireArgc(args, 1, "GET_PLAYER_POS <1|2>") then
			return nil, "Usage: GET_PLAYER_POS <1|2>"
		end
		local idx = assert(playerIndex(args, 1))
	...
	self:register("SET_JOYSTICK_NON_GAMEPAD", function(args)
		if not requireArgc(args, 2, "SET_JOYSTICK_NON_GAMEPAD <1|2> <true|false>") then
			return nil, "Usage: SET_JOYSTICK_NON_GAMEPAD <1|2> <true|false>"
		end
		local idx = assert(playerIndex(args, 1))
...
function CommandHandler:registerActionCommands()
	self:register("INPUT", function(args)
		if not requireArgc(args, 3, "INPUT <1|2> <left|right|up|down|use|start|back|a|b> <down|up>") then
			return nil, "Usage: INPUT <1|2> <left|right|up|down|use|start|back|a|b> <down|up>"
		end
		local idx = assert(playerIndex(args, 1))
	...
	self:register("KEY", function(args)
		if not requireArgc(args, 3, "KEY <1|2> <key> <down|up>") then
			return nil, "Usage: KEY <1|2> <key> <down|up>"
		end
		local idx = assert(playerIndex(args, 1))
		local key = args[2]
		local down = args[3]:lower() == "down"
	...
	self:register("PRESS_KEY", function(args)
		if not requireArgc(args, 2, "PRESS_KEY <1|2> <left|right|up|down|use|start|back|a|b>") then
			return nil, "Usage: PRESS_KEY <1|2> <left|right|up|down|use|start|back|a|b>"
		end
		local idx = assert(playerIndex(args, 1))
		local action = assert(resolveValidatedAction(args[2]))
	...
	self:register("RELEASE_KEY", function(args)
		if not requireArgc(args, 2, "RELEASE_KEY <1|2> <left|right|up|down|use|start|back|a|b>") then
			return nil, "Usage: RELEASE_KEY <1|2> <left|right|up|down|use|start|back|a|b>"
		end
		local idx = assert(playerIndex(args, 1))
		local action = assert(resolveValidatedAction(args[2]))
	...
	self:register("HOLD_KEY", function(args)
		if not requireArgc(args, 3, "HOLD_KEY <1|2> <left|right|up|down|use|start|back|a|b> <duration>") then
			return nil, "Usage: HOLD_KEY <1|2> <left|right|up|down|use|start|back|a|b> <duration>"
		end
		local idx = assert(playerIndex(args, 1))
		local action = assert(resolveValidatedAction(args[2]))
		local duration = numberArg(args, 3, "duration")
		if not duration or duration <= 0 then
			return nil, "Duration must be a positive number"
		end
...
function CommandHandler:registerControlCommands()
	self:register("RESIZE", function(args)
		if not requireArgc(args, 2, "RESIZE <width> <height>") then
			return nil, "Usage: RESIZE <width> <height>"
		end
	...
	self:register("LOAD_MAP", function(args)
		if #args < 1 then
			return nil, "Usage: LOAD_MAP <map_name>"
		end
	...
	self:register("SPAWN_ENTITY", function(args)
		if #args < 3 then
			return nil, "Usage: SPAWN_ENTITY <type> <x> <y> [props_json]"
		end
		local entityType = args[1]
		local x, y = tonumber(args[2]), tonumber(args[3])
		if not x or not y then
			return nil, "x and y must be numbers"
		end
		local props = {}
		if args[4] then
			local ok, parsed = pcall(require("lib.dkjson").decode, args[4])
			if not ok then
				return nil, "Invalid props JSON"
			end
			props = parsed
		end
	...
	self:register("STEP_FRAMES", function(args)
		if not requireArgc(args, 1, "STEP_FRAMES <count>") then
			return nil, "Usage: STEP_FRAMES <count>"
		end
