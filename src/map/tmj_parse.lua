-- Parses Tiled .tmj object layers (objects, properties, and the layers that
-- hold them) into the table shape the vendored map loader (lib/sti) expects.
-- Also owns the template / embedded-tileset resolution that object parsing
...
-- Shared: used by both the map-tileset path (tmj.lua) and template-gid
-- resolution here.
...
function TmjParse.embeddedKeyEquals(a, b)
...
--- Parses a TMJ properties array into a key-value table.
...
--- Parses a layer from TMJ format, dispatching on layer type. Tile layers
--- keep their raw `data` (base64 string or plain gid array) for STI to
