...
local Sensors = require('src.npc.npc_sensors')
...
function NPCBase:update(dt)
    ...
    if Sensors.checkKillZones(self) then return end

    -- Detect nearby players for follow/chase behavior
    Sensors.detectNearestPlayer(self)
...
function NPCBase:isOnGround()
    return Sensors.isOnGround(self)
end

function NPCBase:isWallAhead(direction)
    return Sensors.isWallAhead(self, direction)
end

function NPCBase:isGroundAhead(direction)
    return Sensors.isGroundAhead(self, direction)
end
