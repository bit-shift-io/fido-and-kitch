...
- `F7` — Voronoi dynamic split-screen compositing: renders each player into its own full-window canvas and composites them with a dividing **line** (a point + unit normal, always through screen centre, only rotating — never two sites) that only the shader-facing composite sees; off = single shared auto-zoom camera. Toggles `conf.voronoi`. See `src/camera.lua` (`CameraManager`) below and `docs/adr/0007-centred-split-line-centroid-anchored-panes.md` for the geometry.
...
- `src/map/` — map system modules:
  - `init.lua` — Map class, STI/JSON loading, public API, background map
  - `entity_factory.lua` — Tiled object layer → runtime entity instantiation + ladder annotation. For objects with no `template` ref (runtime-mock objects like replicator spawns), it auto-applies the `res/entities/<type>.tj` default properties so they inherit the same defaults as template-placed instances
  ...
  - `parallax_renderer.lua` — background image layers, zoom-coupled parallax, screen-space draw. Each imagelayer is **zoom-coupled**: drawn at `(1 + allowance) * max(mapW/imgW, mapH/imgH) * cameraScale`, so it always covers the whole projected world rect (never just the screen) and its drawn rect always contains the world rect. Layers anchor to world-center plus a **proportional slide** `allowance * parallax * (reference - mapSize/2) * scale` (per axis), where `reference` is the players' average position (union-bounds midpoint of the player targets passed via `Map:draw2(tx,ty,sx,sy,targets)`, from `InGameState:collectPlayerTargets()`) — this keeps parallax alive at the zoomed-out view, where the camera center is clamped to map-center; when no player rects are supplied it falls back to the camera center recovered from tx/ty/sx/sy. Hard-clamped so a texture edge can never show inside the world (zero/negative slack locks to world-center). Authored `offsetx`/`offsety` are **discarded**. Scissored to the floored projected world rect so the background never bleeds into the Diorama void strips
  - `map_parallax.lua` — pure love/sti-free math for the above: `computeCameraCenter`, `computeZoomT` (clamped [0,1] normalization of camera scale between full-map and closest views), `computeAllowance` (linear lerp between the two shared presets `ZOOMED_OUT_ALLOWANCE = 0.10` / `ZOOMED_IN_ALLOWANCE = 0.30`), `computeCover`, `computeSlide`, `computePlayersCenter` (union-bounds midpoint of player rects, the slide reference). Unit-tested in `tests/unit/map_parallax_test.lua`
  - `tmj.lua` — JSON map parsing into an STI-shaped table (see `src/map/` for the loader); `tj_template.lua`, `tj_tileset.lua` — JSON template/tileset resolution
- `src/camera.lua` — shared auto-zoom camera framing all players. Optional `padding` (world px, default 0) keeps a gutter of void visible around every map edge whenever a map edge is on screen: at the zoom-out limit the view spans `mapW + 2*padding` instead of fitting flush, so the diorama frame always shows. `InGameState` opts in with `padding = 16`; a view narrower than the map is unaffected (no edge on screen). `Camera.MAX_VIEW_TILES` (20) caps how far `follow` mode can zoom out — a floor applied to `scale` in `Camera.computeFraming` after the map-bounds/padding logic runs, never a ceiling on `bw`/`bh`. Overview, level-start, and game-over all bypass the cap via `Camera.fullMapView`/`Camera:computeTargetView`'s early return, so they still reach the true full-map view on any size map.
  - `CameraManager` (same file) is the Voronoi split-screen camera (`conf.voronoi` / F7). It owns one shared "merged" `Camera` plus two per-player pane `Camera`s, and is the single owner of all split geometry — `InGameState` and the shader only consume what it computes, never re-derive it.
    - **Crossover trigger** (`CameraManager:updateSplit`): the split fires at the point where the merged view already draws each player exactly where the split view would. Every frame it compares the players' on-screen separation (world separation × the merged camera's *eased* zoom) against the on-screen separation of the two split-region centroids (`getRegionCentroidSeparation`) — W/2 for a vertical divider, H/2 for a horizontal one. Past that, the screen splits; back inside it, it merges. Because both renderings agree at that point, nothing moves on screen as the divider appears or disappears: the screen simply parts as the players walk away from each other. Two constants (`SPLIT_TRIGGER_SEPARATION_MARGIN` = 1.0, `SPLIT_RELEASE_SEPARATION_MARGIN` = 0.98) carve out a deliberately narrow dead band, present only to stop the state thrashing on the boundary — a wide one would just delay an already-seamless transition. Measured in screen pixels, which is safe *because both sides are screen-space*; the zoom cap (`Camera.MAX_VIEW_TILES`) now only governs how far the merged camera may zoom out, and is not part of the trigger. Only evaluated in `follow` mode — meaningless in overview/game-over.
    - **F7 line behaviour**: the dividing line (`CameraManager:getSplitLine()`) is a point + unit normal in screen pixels, always pinned to screen centre — it only rotates, never travels (a signed-offset seam exists for a future slide but ships at zero). The angle follows `atan2` of the players' world separation, eased with a max-rotation clamp (~150°/s) and a ~5° dead zone against jitter. Each pane camera then anchors its player at the **centroid** of that player's half (the split line's half-plane clipped to the screen rect, via a general convex-polygon centroid) rather than the pane's midpoint — since a convex region's centroid always lies inside it, a player can never render on the wrong side of the line, by construction rather than tuning. See `docs/adr/0007-centred-split-line-centroid-anchored-panes.md` for the full geometric model and its trade-offs.
- `src/diorama.lua` — **Diorama**: when the camera's view extends past the map bounds, fills the out-of-world screen strips with a generic tiling void (`res/img/diorama/diorama_void.png`) and draws a decorative world-space frame (edge tiles + 4 corner ornaments + deterministic interstitial ornaments) sitting on a frame line pushed `frame.outset` out from the world boundary (so the frame hugs the border instead of overlapping the playfield). One global stateless config `Diorama.config` (no seeds): `void.img`; `frame.tileSize` (tiling band width, default 32), `frame.outset` (default 14), `frame.tiles.{top,bottom,left,right}` (top/bottom share one horizontal tile texture, left/right share one vertical), `frame.ornaments.corners.{topLeft,topRight,bottomLeft,bottomRight}` (corners are ornaments — same draw stage, each with its own `scale` and an optional `offset` that moves it diagonally outward), `frame.ornaments.{top,bottom,left,right}` = flat lists of `{img, pos, offset?, scale?}` (discrete ornaments, one list entry per piece — never tiled — each placed at its `pos`, a 0..1 percentage along the edge: 0 = left/top corner, 1 = right/bottom corner; optional `offset` is world px perpendicular to the edge — positive pushes the piece away from the playfield into the void, negative toward it, 0 = centred on the frame line; optional `scale` is an `{x, y}` pair, default `{x=1, y=1}` — negative values flip along that axis, arbitrary values stretch). Tiling model: each edge texture's **short dimension maps 1:1 to `tileSize`** (the band width) and its **long dimension runs along the edge, repeating until the whole edge is tiled** (`computeTileCount` = `ceil(edgeLen/tileLen)`, overruns covered by the corner ornament); textures are authored landscape for top/bottom edges and portrait for left/right. Draw order in `InGameState:draw`: void tiling → parallax bg → world tiles → **frame (tiles, then corners, then ornaments)** → entities. Geometry contract: the projected world rect is `{x=floor(tx), y=floor(ty), w=mapW*sx, h=mapH*sy}` (floored origin must match `ParallaxRenderer:drawMainLayers`' translate); use the same floored origin for the void strips, the parallax scissor, and the frame transform or a hairline strip shows at fractional `tx/ty`. Pure geometry (worldScreenRect/computeVoidRects/computeTileCount/computeFrame) exposed via `Diorama._internal` for headless unit tests; draw methods no-op without `love.graphics`, and missing art is skipped with a memoized `Log.warn` so the game runs before assets land. e2e pre-seeds `AssetManager.textures[path]` with placeholder images to verify geometry
...
- `src/entities/` — map entity implementations; Tiled object `type` must match a filename here (`key` → `src/entities/key.lua`). Sprite art now comes from the entity's `.tj` template — the `image` is resolved from the template's **inline tileset tile** (what the editor previews), and the rest via `src/entities/sprite_props.lua` (`SpriteProps.fromObject(object)` reads merged `object.properties` and returns the `Sprite{}` art fields actually present — `frames`, `duration`, `loop`, `playing`, `scaleX`/`scaleY`), so entity sprite bodies carry no art literals and templates carry no duplicated `image` property; position/shape/facing/finish/timeline choices stay in the entity
...
- `res/map/` — Tiled `.tmj` sources loaded directly; the game runs `.tmj` via `src/map/tmj.lua`
- `res/entities/` — Tiled authoring assets: the Tiled project file (`fido-and-kitch.tiled-project`), object templates (`*.tj`, flat) and external tilesets (`tileset_*.tsj`, flat, `tileset_`-prefixed). Maps reference these via `../entities/...` relative paths; templates reference their tileset as a bare filename; tilesets reference images via `../img/...`. All are parsed at runtime from map/template references (`src/map/tj_template.lua`, `tj_tileset.lua`) — keep them inside `res/` so they ship. Templates are the **single source of truth** for an entity's default + sprite properties: the sprite `image` comes from the template's **inline tileset tile** (what the editor previews), exposed at load as `tilesetImage` and injected into the merged `object.properties.image` by `tmj.lua parseObject` (and `entity_factory:_mergeTemplateProps` for template-less runtime-mock objects); the rest — `frames`, `duration`, `loop`, `playing`, `scaleX`/`scaleY`, plus behaviour like `speed`/`endBehavior` — are template props (`file`-typed values `../img/...` are path-normalised to `res/img/...` at load, same `format_path` pattern as `tj_tileset.lua`). Any instance can override any property.
- `res/bg/` — reusable parallax background presets (image-layer maps, `.tmj`), referenced by the `background` map property and loaded via the `res/bg/` prefix in `src/map/init.lua`
...
- **An entity's `.tj` template must stay in sync with the code that reads its properties.** `res/entities/<type>.tj` (see "Layout" above) is the single source of truth for that entity's default properties — reading `object.properties.<name>` in `src/entities/<type>.lua` without a matching property declared in the template means every Tiled-placed (not runtime-mock) instance is missing it unless an author overrides it by hand per-instance. When adding, renaming, or removing a property an entity reads, update its `.tj` template in the same change — add new ones (object-type refs default to `value: 0`, meaning "unset"; `map:getObjectById(0)` correctly resolves to `nil`, so `if object.properties.target then` alone isn't a safe existence check — compare `.id ~= 0` or rely on the resolved lookup being nil), remove ones that are no longer read, and don't leave stale properties (e.g. a leftover `path` property on an entity that never reads `path`) that misleads whoever authors instances in Tiled next.
