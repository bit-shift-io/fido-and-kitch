...
- `res/editor/` — Tiled authoring assets: the Tiled project file (`fido-and-kitch.tiled-project`), object templates (`*.tj`, flat) and external tilesets (`tileset_*.tsj`, flat, `tileset_`-prefixed). Maps reference these via `../editor/...` relative paths; templates reference their tileset as a bare filename; tilesets reference images via `../img/...`. All are parsed at runtime from map/template references (`src/map/tj_template.lua`, `tj_tileset.lua`) — keep them inside `res/` so they ship
